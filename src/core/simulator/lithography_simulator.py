...
class LithographySimulator:
    ...
    def simulate(self, mask):
        ...
        # Create frequency grid for Fourier optics
        ...
        # Loop over all illumination source points
        for (source_x, source_y, source_intensity) in source_points:

            #Shift the pupil function by the source location
            pupil_filter = self.get_pupil_filter(freq_x - source_x, freq_y - source_y)
            
            #Apply the pupil filter in the frequency domain
            filtered_ft = mask_ft * pupil_filter
            total_filtered_ft += filtered_ft * source_intensity

            #Compute the field at the wafer plane
            wafer_field = np.fft.ifft2(np.fft.ifftshift(filtered_ft))
            
            # Accumulate intensity contributions (squared magnitude)
    ...
    def get_resist_profile(self, intensity):
        ...
        #Perform Gaussian blur for basic resist diffusion
        blurred_intensity = gaussian_filter(intensity, sigma=sigma)

        #Normalise intensity, note that the intensity may be zero everywhere
        ...
        #Sigmoid thresholding for differentiability TODO: is this still needed?
    ...
    def get_pupil_filter(self, freq_x, freq_y):
        ...
        #Complete the cut-off pupil function with a sigmoid so it is differentiable
        ...
        #Defocus abberation (simple approximation)
