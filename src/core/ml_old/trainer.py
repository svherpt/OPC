...
def train():
    ...
    # Diagnostic: Check data ranges and shapes
    print("\nChecking data shapes and ranges...")
    sample_mask, sample_illum, sample_int, sample_res = train_dataset[0]
    print(f"Mask shape: {sample_mask.shape} | range: [{sample_mask.min():.4f}, {sample_mask.max():.4f}]")
    print(f"Illumination shape: {sample_illum.shape} | range: [{sample_illum.min():.4f}, {sample_illum.max():.4f}]")
    print(f"Intensity shape: {sample_int.shape} | range: [{sample_int.min():.4f}, {sample_int.max():.4f}]")
    print(f"Resist shape: {sample_res.shape} | range: [{sample_res.min():.4f}, {sample_res.max():.4f}]")
    
    # Test forward pass with a batch
    print("\nTesting forward pass...")
    test_batch = torch.stack([sample_mask for _ in range(2)])
    test_illum = torch.stack([sample_illum for _ in range(2)])
    print(f"Batch mask shape: {test_batch.shape}")
    print(f"Batch illum shape: {test_illum.shape}")
    ...
    model.eval()
    with torch.no_grad():
        test_mask = torch.stack([sample_mask for _ in range(2)])
        test_illum = torch.stack([sample_illum for _ in range(2)])
        pred_int, pred_res = model(test_mask, test_illum)
        print(f"Input mask shape: {test_mask.shape}")
        print(f"Input illum shape: {test_illum.shape}")
        print(f"Output intensity shape: {pred_int.shape}")
        print(f"Output resist shape: {pred_res.shape}")
        print(f"Expected: mask=Bx1x256x256, illum=Bx1x32x32, outputs=Bx1x256x256")
    ...
    trainer.train(
        ...
        viz_every=1, 
        n_viz_samples=6,
        # resume_from='./checkpoints/checkpoint_epoch_100.pth'  # Adjust to actual checkpoint name
    )
