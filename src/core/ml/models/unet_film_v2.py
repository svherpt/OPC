# src/core/ml/models/unet_film_v2.py
import torch
...
@register_model("unet_film_v2")
class UNetFiLMV2(nn.Module):
    """Deeper UNet with 4 encoder/decoder levels and FiLM conditioning, predicting wafer intensity and resist profile."""

    def __init__(self, config):
        """Initialise UNetFiLMV2 from config with keys: channels (default 32), illum_dim (default 128)."""
        super().__init__()
        ...
        self.enc4 = ConvBlock(ch * 4,  ch * 8,  illum_dim)

        self.pool = nn.MaxPool2d(2)

        # Bottleneck
        self.bottleneck = ConvBlock(ch * 8, ch * 16, illum_dim)

        # Decoder (4 levels)
        self.up4  = nn.ConvTranspose2d(ch * 16, ch * 8, kernel_size=2, stride=2)
        self.dec4 = ConvBlock(ch * 16, ch * 8,  illum_dim)
    ...
    def forward(self, mask, illum_q):
        ...
        # Encoder
        e1 = self.enc1(mask,           illum_emb)  # [B, ch,    128, 128]
        e2 = self.enc2(self.pool(e1),  illum_emb)  # [B, ch*2,   64,  64]
        e3 = self.enc3(self.pool(e2),  illum_emb)  # [B, ch*4,   32,  32]
        e4 = self.enc4(self.pool(e3),  illum_emb)  # [B, ch*8,   16,  16]

        # Bottleneck
        b  = self.bottleneck(self.pool(e4), illum_emb)  # [B, ch*16,  8,   8]

        # Decoder with skip connections
        d4 = self.dec4(torch.cat([self.up4(b),  e4], dim=1), illum_emb)
        d3 = self.dec3(torch.cat([self.up3(d4), e3], dim=1), illum_emb)
