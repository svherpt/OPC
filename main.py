...
def simulate(maskID):
    ...
    random_mask = masks.read_mask_from_img(f'example_masks/{maskID}.glp.png', **sim_config)
...
#     # data_augmenter.main()
#     # trainer.train()
#     # activity_learner.main(data_dir='active_learning_data')
#     # simulate()
    
#     # random_masks = masks.get_dataset_masks('./data/example_masks', 1, **sim_config)
...
#     # test_ML_model()
#     optimize_model_multihead(model_path='./checkpoints/old/best_model.pth')

#     # visualize_dataloader_sample('./data/augmented_massive', split="train", batch_size=1, index_in_batch=0)

# def simulate():
#     litho_sim = simulator.LithographySimulator(sim_config)
#     source_illumination = illumination.create_quadrant_source(sim_config)  # shape: (Npupil, Npupil)

#     random_mask = masks.read_mask_from_img('example_masks/343.glp.png', **sim_config)
...
#     num_samples = 4
#     maskList = [masks.get_random_dataset_mask('example_masks', **sim_config) for _ in range(num_samples)]
#     illumList = [illumination.read_random_illumination_quarter('augmented_massive/test/illuminations', **sim_config) for _ in range(num_samples)]
...
#         full_illumination = illumination.quadrant_to_full(illum)   
...
#         upsampled_illum = illumination.upsample_illumination(full_illumination, target_size=mask.shape[0])
...
# def optimize_model_multihead(model_path):
#     opt = SourceMaskOptimizer(modelClass=LithographyUNet, 
#                    modelPath=model_path, 
#                    device='cuda')

#     # target_resist = masks.read_mask_from_img('example_masks/10605.glp.png', mask_grid_size=256)
#     # target_resist = masks.read_mask_from_img('example_masks/572.glp.png', mask_grid_size=256)
#     target_resist = masks.read_mask_from_img('example_masks/572.glp.png', mask_grid_size=256)



#     # This nowt works from zeros with source-mask optimization!
...
#     print(np.sum(optimized_illum), np.min(optimized_illum), np.max(optimized_illum))
